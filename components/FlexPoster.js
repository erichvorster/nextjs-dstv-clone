...
const Poster = ({ result }) => {
  ...
  return (
    <Link href={`watch/${result.id}`} key={result.id}>
      <div className="h-40 w-28 lg:h-52 lg:w-36 relative mx-1 my-1 shrink-0 hover:scale-105 transition-transform ease-in-out 1s border border-gray-800 group cursor-pointer">
        <Image
        ...
        <div className=" p-4">
          <div className=" z-50 ">
            <div className="flex justify-between">
              <div className="h-5 w-6 relative ml-2 mb-2 opacity-0 group-hover:opacity-100 duration-300 z-50">
                <Image className="" src={mnet} layout="fill" alt="mnet"/>
              </div>
              <div className="flex">
          ...
          <div className="absolute bottom-3 left-2 z-50 opacity-0 group-hover:opacity-100 duration-300">
            <button className="h-6 w-16 lg:h-8 lg:w-32 text-xs bg-white text-black rounded-full cursor-pointer hover:bg-sky-500 hover:text-white duration-300">
              Watch
            </button>
          </div>
        </div>
