...
const Poster = ({ result }) => {
  ...
  return (
    <Link href={`watch/${result.id}`} key={result.id}>
      <div className="h-44 w-28 lg:h-52 lg:w-36 relative mx-2 my-2 shrink-0 hover:scale-105 transition-transform ease-in-out 1s border border-gray-800 group cursor-pointer">
        <Image
        ...
        <div className="hidden md:inline-block absolute z-50 top-0 left-0 w-full h-full  bg-black/75 opacity-0 group:hover:h-full group-hover:opacity-70 duration-200 p-4 z-10 ">
          <div className="">
            ...
            <div className="absolute bottom-3 left-2">
              <button className="h-6 w-12 lg:h-8 lg:w-32 text-xs bg-white text-black rounded-full cursor-pointer hover:bg-sky-500 hover:text-white duration-300">
                Watch
              </button>
            </div>
          </div>
